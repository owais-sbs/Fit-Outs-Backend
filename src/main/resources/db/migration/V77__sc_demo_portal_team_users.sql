-- Demo SC portal team (password 123456) linked to the same organization as subcontractor@fitouts.demo.
-- SC Admin remains subcontractor@fitouts.demo; these four cover the other portal roles.
...
BEGIN
    ...
    IF v_admin_account_id IS NULL THEN
        RAISE NOTICE 'V69 skipped: demo SC admin not found';
        RETURN;
    END IF;
    ...
    IF v_org_uuid IS NULL THEN
        RAISE NOTICE 'V69 skipped: no SC organization for demo subcontractor';
        RETURN;
    END IF;
    ...
    RAISE NOTICE 'V69 demo SC portal team seeded for organization %', v_org_uuid;
END $$;
